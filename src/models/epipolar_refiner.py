...
def groupwise_epipolar_correlation(
    ...
    *,
    ...
    ``(u_right, v_right) = (u - predicted_disparity_hr_px - delta, v)``.
    ...
    source_v = row.view(1, 1, height, 1).expand(
        batch, candidate_count, height, width
    )
    finite_coordinate = torch.isfinite(source_u)
    candidate_valid_mask = finite_coordinate & (source_u >= 0.0) & (
