"""Full-pixel hypothesis supervision; no single GT label per coarse cell."""
import torch
...
def hypothesis_loss(output,record):
    ...
    for t in range(count-2,count):
        ...
        acceptable=(initial_error<=best+.1)&cv
        current_tie=initial_error[:,:1]<=best+.1;only_current=torch.zeros_like(acceptable);only_current[:,0]=True
        acceptable=torch.where(current_tie,only_current,acceptable)
