...
from models.codd_metric_aligned_loss import codd_loss
...
def main():
    ...
    p.add_argument('--profiles',default='codd,codd_regret')
    ...
    for component in args.components.split(','):
      for profile in args.profiles.split(','):
        ...
        arm=f'learned_codd_margin01_{profile}';outdir=args.output_dir/arm;outdir.mkdir(exist_ok=True)
        receipt={'arm':arm,'profile':profile,'coefficients':PROFILES[profile],'components':flags,'steps':args.steps,'seed':args.seed,'batch_size':args.batch_size,
            ...
            'architecture':'learned_codd_component','fusion_margin_px':.1,'reset_margin_px':5.,
            'loss_source_sha256':sha(ROOT/'src/models/codd_metric_aligned_loss.py'),'v1_shift':v1_payload['logit_shift'],
            'loss_origin':'CODD FusionLoss, margins 5 and 0.1 HR px, tie regularizer 0.2; no GT max-disparity filtering',
