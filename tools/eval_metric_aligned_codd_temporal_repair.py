...
def safety_values(pred,record,v1_prediction):
    ...
    margin_metrics={}
    advantage=be-he
    for label,lo,hi in (('01_to_02',.1,.2),('02_to_1',.2,1.),('1_to_5',1.,5.),('over_5',5.,float('inf'))):
        selected=opportunity&(advantage>lo)&(advantage<=hi)
        margin_metrics['opportunity_'+label+'_recovery_rate']=scalar_metric((be-err>.1).float(),selected)
        margin_metrics['opportunity_'+label+'_error_px']=scalar_metric(err,selected)
    return {
      **margin_metrics,
...
def main():
    ...
    for checkpoint in args.checkpoints:
        ...
        model=LearnedCoddTemporalRepair(v1,v1_shift=cfg['v1_shift']).to(device).eval();model.load_state_dict(payload['model'],strict=True)
