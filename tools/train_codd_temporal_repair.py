...
def main():
    ...
    for component in args.components.split(','):
      for profile in args.profiles.split(','):
        ...
        flags={}
        model=CoddTemporalRepair(v1,v1_shift=v1_payload['logit_shift']).cuda().train()
        optimizer=torch.optim.Adam([v for v in model.parameters() if v.requires_grad],lr=2e-4,weight_decay=1e-5)
        arm=f'codd_component_{profile}';outdir=args.output_dir/arm;outdir.mkdir(exist_ok=True)
        receipt={'arm':arm,'profile':profile,'coefficients':PROFILES[profile],'components':flags,'steps':args.steps,'seed':args.seed,'batch_size':args.batch_size,
            ...
            'architecture':'codd_component','v1_shift':v1_payload['logit_shift'],
            ...
            'training_source_sha256':sha(Path(__file__)),'model_source_sha256':sha(ROOT/'src/models/codd_temporal_repair.py')}
