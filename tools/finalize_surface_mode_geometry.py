...
ROOT=Path(__file__).resolve().parents[1];RUN=ROOT/'runs/metric_stereo_video/surface_mode_geometry_20260907';CACHE=Path('/tmp/vggt_native_stereo_20260907')
...
def main():
    ...
    checkpoints=[str(RUN/n/'final.pt') for n in ('wta','mode_pool')];launch=[sys.executable,'-m','torch.distributed.run','--standalone','--nproc_per_node=8']
    commands=[('evaluation',launch+['tools/eval_surface_mode_geometry.py','--cache',str(CACHE),'--original-cache','/tmp/vggt_temporal_repair_20260907',
      ...
      ('raw_checks',launch+['tools/check_surface_mode_geometry_raw.py','--cache',str(CACHE),'--config','runs/metric_stereo_video/formal_a5_seed42/resolved_config.yaml',
      ...
      ('unit_tests',[sys.executable,'-m','pytest','-q','tests/test_surface_mode_geometry.py']),
      ('report',[sys.executable,'tools/report_surface_mode_geometry.py','--run-dir',str(RUN)])]
    ...
    archive=RUN/'source_final';sources=[*sorted((ROOT/'src/models').glob('surface_mode*.py')),*sorted((ROOT/'tools').glob('*surface_mode_geometry*.py')),ROOT/'tests/test_surface_mode_geometry.py',ROOT/'docs/surface_mode_geometry_protocol.md']
    hashes={}
    ...
    summary={'status':'COMPLETE','trained_arms':['wta','mode_pool'],'updates_per_arm':2000,'validation_endpoints_per_arm':1294,
