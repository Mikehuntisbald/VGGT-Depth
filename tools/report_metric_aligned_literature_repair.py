...
def main():
    ...
    reports={k:json.loads((a.run_dir/f'evaluation_{k}/metrics.json').read_text()) for k in ('supervision','learned','metric_aligned')}
    ...
    for group,report in reports.items():
        ...
        for metric,value in report['metrics']['v1'].items():
            if metric in baseline:assert baseline[metric]==value['value']
        for name,acceptance in report['acceptance'].items():
            if name in arms:continue
    ...
    (a.run_dir/'extended_results.json').write_text(json.dumps(result,indent=2)+'\n')
    points=[]
    ...
    lines=['# 论文指导下的 temporal 改进：实训验收','',f'{len(arms)} 组实验均完成 12,500 步训练及全部 1,294 个验证端点评估。五项同时通过的版本：{accepted or "无"}。','',
    ...
    margin=reports['metric_aligned']['metrics']
    lines += ['', '追加的 margin01 对照只将 fusion 的近似等优阈值从 1 调整为原验收定义的 0.1 px，reset 仍为 5 px。这个适配发生在六组结果之后，不属于事前确认。', '',
              '| 固定历史优势区域 | 像素数 | v1 恢复 % | 原 CODD+特征 % | margin01+特征 % | margin01+退化惩罚 % |',
              '|---|---:|---:|---:|---:|---:|']
    breakdown={}
    for label in ('01_to_02','02_to_1','1_to_5','over_5'):
        key='opportunity_'+label+'_recovery_rate'
        names=('v1','learned_codd_codd','learned_codd_margin01_codd','learned_codd_margin01_codd_regret')
        breakdown[label]={n:margin[n][key] for n in names}
        lines.append('| '+label+' | '+str(margin['v1'][key]['count'])+' | '+' | '.join(f"{100*margin[n][key]['value']:.4f}" for n in names)+' |')
    (a.run_dir/'opportunity_margin_breakdown.json').write_text(json.dumps(breakdown,indent=2)+'\n')
