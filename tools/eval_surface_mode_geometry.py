...
def main():
    ...
    for checkpoint in a.checkpoints:
        ...
        if cfg['cache_lineage_sha256']!=lineage or cfg['model_source_sha256']!=digest(ROOT/'src/models/surface_mode_geometry.py') or cfg['shared_model_source_sha256']!=digest(ROOT/'src/models/hypothesis_geometry.py'):raise RuntimeError('trained source/input lineage changed')
        model=SurfaceModeGeometry(mode_pool=name=='mode_pool',radius_px=cfg['surface_radius_px']).to(device).eval();model.load_state_dict(payload['model'])
        ...
        for name in names:
            ...
            if name in ('A5','v1'):
                ...
                extra={'candidate_oracle_epe_px':scalar_metric(oracle.clamp_max(10),valid),
                  ...
                  'history_mixture_weight_mean':scalar_metric(detail['mixture_weights'][:,HISTORY_START:].sum(1,keepdim=True),valid),
                  'history_participation_rate':scalar_metric((detail['mixture_weights'][:,HISTORY_START:].sum(1,keepdim=True)>1e-4).float(),valid)}
                if row['dataset_index'] in (2,24,29):
                    ...
                    cases.append({**row,'variant':name,'x':x,'y':y,'GT':float(gt[0,0,y,x]),'A5':float(c['base'][0,0,y,x]),'v1':float(v1p[0,0,y,x]),'prediction':float(pred[0,0,y,x]),
                      ...
                      'candidate_predictions':detail['hypotheses'][0,:,y,x].tolist(),'candidate_valid':detail['candidate_valid'][0,:,y,x].tolist(),'probabilities':detail['probabilities'][0,:,y,x].tolist(),'mixture_weights':detail['mixture_weights'][0,:,y,x].tolist()})
    ...
    if ctx.primary:
        ...
        result={'status':'SMOKE_COMPLETE' if a.max_samples else 'COMPLETE','samples':len(rows),'metrics':metrics,'models':configs,'accuracy_coverage':curves,
          ...
          'acceptance':'report tradeoffs; no five-way intersection requirement; repeated development benchmark','candidate_metric_note':'best individual candidate is not an oracle bound for within-surface interpolation; selected_candidate is the mode center, while mixture_weights show actual source participation','elapsed_s':time.monotonic()-started}
