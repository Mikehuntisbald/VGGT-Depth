...
from models.surface_mode_loss import surface_mode_loss
...
def main():
    ...
    p.add_argument('--variant',choices=('wta','mode_pool'),required=True);p.add_argument('--steps',type=int,default=2000);p.add_argument('--smoke',action='store_true');p.add_argument('--initial-checkpoint',type=Path,required=True)
    ...
    model=SurfaceModeGeometry(mode_pool=a.variant=='mode_pool').to(ctx.device).train()
    initial=torch.load(a.initial_checkpoint,map_location='cpu',weights_only=False);model.load_state_dict(initial['model'])
    ...
    receipt={'architecture':'surface_mode_geometry','initial_checkpoint':str(a.initial_checkpoint),'initial_checkpoint_sha256':digest(a.initial_checkpoint),'surface_radius_px':2.0,'variant':a.variant,'steps':a.steps,'world_size':ctx.world_size,'seed':42,'training_endpoints':len(paths),'development_endpoints':len(identities)-len(paths),
      ...
      'initialization':'same completed memory decoder checkpoint for both arms; all FFS/VGGT observations fixed; old A5 geometry and v1 absent',
      ...
      'model_source_sha256':digest(ROOT/'src/models/surface_mode_geometry.py'),'shared_model_source_sha256':digest(ROOT/'src/models/hypothesis_geometry.py'),'loss_source_sha256':digest(ROOT/'src/models/surface_mode_loss.py'),'trainer_sha256':digest(Path(__file__)),
    ...
    for step in range(a.steps):
        ...
        with torch.autocast('cuda',dtype=torch.bfloat16):out=wrapped(record['inputs']);loss,parts=surface_mode_loss(out,record)
