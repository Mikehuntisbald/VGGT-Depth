...
## D-031 — Correct pixel centres and move FFS ownership to the measurement domain in v3.1

- Date: 2026-09-01
- Status: implementation verified; replacement formal training pending
- Context: FFS observations are resized with bilinear `align_corners=False`, but the first v3 implementation formed LR intrinsics as `K_hr/s`. Its principal point was therefore displaced by 0.25 LR px at x2 and 0.375 LR px at x4. Disparity itself remains correct because the common left/right offset cancels, but rays, 3D reconstruction, source/target reprojection, hidden transport and fractional phase do not. V2 also treated trusted LR FFS bilinear interpolation as immutable at every HR pixel, which prevents the subpixel detail the model is meant to reconstruct. Finally, the multi-age merge could retain four redundant candidates and aggregated them before observing current RGB/FFS evidence.
- Decision: create the explicit v3.1 config/checkpoint lineage. When `pixel_center_contract=align_corners_false_half_pixel_v3_1`, resize intrinsics with `f'=sf` and `c'=(c+0.5)s-0.5`; use this contract for rays and every LR source/target transport while keeping HR teacher correspondence in HR coordinates. Generate the learned LR RGB grid by the same align-corners-false resampling before applying the registered downsampling-convolution weights at stride one; the original stride-two path remains legacy-only. Preserve FFS metric ownership by projecting trusted predictions onto the exact LR-centre measurement operator, not by overwriting the full HR bilinear field; bound remaining HR detail by confidence and FFS boundary uncertainty, and require VGGT/history metric support for hole completion. Select top-K with per-age quota and phase redundancy control, keep rank zero physical, separate front/same-surface metric candidates from back-layer context, and let a current-RGB/current-geometry-conditioned attention choose candidates. Carry an 8-channel projection of each age's warped final ConvGRU feature as a candidate value while age one alone initializes recurrent state. Derive a window-level continuous pose-quality score from the cache-bound baseline-CV, rotation, photometric and depth residual/threshold ratios without replacing the boolean quality gate. Expose separate retained-set, transport-prior and learned-attention diagnostics so selection diversity cannot be conflated with learned use. Activation-checkpoint only the learned candidate-attention core during training so the formal 4x2 peak-memory delta stays within the predeclared 5% gate without changing inference semantics.
- Compatibility: absence of every v3.1 field retains v1/v2/original-v3 model and transport behavior. The original v3 configs, 92-key state dict and Stage-C output path are unchanged. V3.1 adds no scale token/head/loss, retains disparity output and `Z=fx*B/d`, and remains below 12M parameters.
- Consequence: the interrupted original-v3 seed-42 A0/A1 artifacts and partial A2 are preserved for provenance but invalid for promotion because they used the inconsistent pixel-centre contract. They must not be resumed or compared as v3.1 evidence. The decision reader fail-closes on lineage/output relabelling, v3.1 behavior-config drift, selected primary/A3 checkpoint SHA drift, or incomplete aggregate/per-record top-K interpretation receipts; per-record invalid diagnostic domains are explicit JSON nulls, never omitted fields. Until the new v3.1 runner completes its bound training/evaluation and publishes `decision.json`, learned rays and temporal-pose conditioning are **NO-GO / NOT ESTABLISHED**; only implementation readiness is GO.
- Evidence: `resize_intrinsics_align_corners_false`, `project_ffs_measurement_ownership_v3_1`, `TOPK_DIVERSITY_V31_CONTRACT`, `CurrentConditionedTopKAttention`, `configs/*v3_1.yaml`, `tests/test_half_pixel_*`, `tests/test_measurement_ownership_v3_1.py`, `tests/test_topk_splat_v31.py`, `tests/test_current_conditioned_history_v31.py`, and `tests/test_v31_integration.py`.

## D-032 — Spring v2 is a distinct real-GT supervision lineage

- Date: 2026-09-02
- Status: implementation ready; caches/training pending completed dataset extraction
- Context: Spring provides Full-HD rectified stereo images, per-frame intrinsics and 4K `disp1` ground truth. The official contract states that disparity vectors remain in Full-HD image-pixel units despite the 2x-per-axis storage grid. Reusing the `ffs-teacher` identity or scaling values by two would corrupt both scientific claims and metric geometry.
- Decision: build sequence-disjoint JSONL manifests from official Spring v2 folders, use the left camera as reference, baseline 0.065 m, and retain variable per-frame K. Sample 4K GT by `[::2,::2]` onto the RGB/model HR grid without value scaling; zero sky and non-finite values are invalid. Cache it under component `spring-ground-truth`, require an exact opt-in supervision config in checkpoints/evaluation, and report it as rendered synthetic GT. Continue using frozen FFS only as the LR observation owner and VGGT only as a prior/pose source. Generate the v3 rectified-rig sidecar from audited official orthoparallel P/K structure.
- Compatibility: absent `supervision` preserves every legacy config and `ffs-teacher` expectation. Spring checkpoints cannot be evaluated with pseudo-GT configs/caches, or vice versa. Official test images have no public GT and are manifest/cache-ready for inference, but local acceptance metrics use a sequence-held-out subset of the official train split.
- Evidence: `src/data/spring.py`, `tools/build_spring_manifest.py`, `tools/cache_spring_gt.py`, `configs/spring_*_v3_1.yaml`, and `tests/test_spring_data.py`.
